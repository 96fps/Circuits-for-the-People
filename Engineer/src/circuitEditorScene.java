		...
		for(int i=0; i<(int)dim.x; i++){
			for(int j=0; j<(int)dim.y; j++){
				if(metalpower[0][i][j] && circ.layer[i][j]){
					...
					//propagate right
					if(metalpower[0][i][j] && circ.horiz[i][j]){
					...
					//propagate left
					if(metalpower[0][i][j] && circ.horiz[(i+(int)dim.x-1)%(int)dim.x][j]){
						metalpower[1][(i-1)%(int)dim.x][j]=true;
					...
					//propagate down
					if(metalpower[0][i][j] && circ.verti[i][j]){
					...
					//propagate up
					if(metalpower[0][i][j] && circ.verti[i][((int)dim.y+j-1)%(int)dim.y]){
						metalpower[1][i][(j-1)%(int)dim.y]=true;
		...
		for(int i=0; i<(int)dim.x; i++){
			for(int j=0; j<(int)dim.y; j++){
				if(circ.layer[i][j]){
					...
					else g.setColor(new Color(255,255,255,96));
